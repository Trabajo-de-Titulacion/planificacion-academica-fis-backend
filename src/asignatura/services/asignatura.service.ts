...
export class AsignaturaService {
    ...
    async crearUnaAsignatura(asignaturaDto: AsignaturaDto) {
        const existenciaAsignatura = await this.obtenerAsignaturaPorCodigo(asignaturaDto.codigo);
        if (existenciaAsignatura instanceof NotFoundException) {
            ...
            return {
                mensaje: 'Se creó la asignatura ' + asignaturaDto.codigo + ' - ' + asignaturaDto.nombre + ' existosamente.'
            }
        } else {
            return {
                mensaje: 'La asignatura ' + asignaturaDto.codigo + ' - ' + asignaturaDto.nombre + ' ya se encuentra registrada.'
            }
        }
    }
    ...
    async actualizarAsignaturaPorID(idAsignatura: string, asignaturaDto: AsignaturaDto): Promise<UpdateResult | NotFoundException> {
        const asignatura = await this.obtenerAsignaturaPorID(idAsignatura);
        if (asignatura) {
            return await this.asignaturaRepository.update(idAsignatura, asignaturaDto);
        } else {
            return new NotFoundException(`No existe la asignatura con id ${idAsignatura}`);
        }
    }
    ...
    async eliminarAsignaturaPorID(idAsignatura: string): Promise<DeleteResult | NotFoundException> {
        const asignatura = await this.obtenerAsignaturaPorID(idAsignatura);
    ...
    async obtenerAsignaturaPorCodigo(codigoAsignatura: string): Promise<AsignaturaEntity | NotFoundException> {
        const asignatura = await this.asignaturaRepository.findOne({ where: { codigo: codigoAsignatura } });
        if (asignatura) {
            return asignatura;
        } else {
            return new NotFoundException(`No existe la asignatura con el código ${codigoAsignatura}`);
        }
    }
    ...
    async obtenerAsignaturaPorID(idAsignatura: string): Promise<AsignaturaEntity | NotFoundException> {
        const asignatura = await this.asignaturaRepository.findOne({ where: { id: idAsignatura } });
        if (asignatura) {
            return asignatura;
        } else {
            return new NotFoundException(`No existe la asignatura con el código ${idAsignatura}`);
        }
    }
