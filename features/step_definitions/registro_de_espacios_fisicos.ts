...
Given('que existe un espacio fisico llamado {string}', async function (nombreEspacioFisico: string) {
    ...
    this.espacioFisicoExistente = new EspacioFisicoDTO(nombreEspacioFisico, this.laboratorio.id, 25);
    await getRepository(EspacioFisicoEntity).save(this.espacioFisicoExistente);
});

When('se agrega un espacio fisico llamado {string}', async function (nombreEspacioFisico: string) {
    this.espaciosFisicosController = await this.app.get(EspaciosFisicosController);

    this.nuevoEspacioFisico = new EspacioFisicoDTO(nombreEspacioFisico, this.laboratorio.id, 25);
    
    await this.espaciosFisicosController.crearEspacioFisico(this.nuevoEspacioFisico);
});

Then('al consultar la base de datos se observan {string} registros.', async function (numeroRegistros: number) {
    // Se consulta a la base de datos y se obtiene lo almacenado
    this.registrosAlmacenados = await this.espaciosFisicosController.obtenerEspaciosFisicos();
...
After("@espaciosFisicosPruebaCrearUnEspacioFisico", async function () {
    ...
    await getRepository(EspacioFisicoEntity).delete({ nombre: this.espacioFisicoExistente.nombre });
    // Borrar registro creado en el Cuando
    await getRepository(EspacioFisicoEntity).delete({ nombre: this.nuevoEspacioFisico.nombre });
...
Given('que existe un espacio físico llamado BetaPrueba', async function () {
    ...
    this.espacioFisicoExistente1 = new EspacioFisicoDTO("BetaPrueba", this.laboratorio.id, 25);
    await this.repository.save(this.espacioFisicoExistente1);
});
Given('existe un espacio físico llamado AlfaPrueba', async function () {
    this.espacioFisicoExistente2 = new EspacioFisicoDTO("AlfaPrueba", this.laboratorio.id, 25);
    await this.repository.save(this.espacioFisicoExistente2);
});
Given('existe un espacio físico llamado GammaPrueba', async function () {
    this.espacioFisicoExistente3 = new EspacioFisicoDTO("GammaPrueba", this.laboratorio.id, 25);
...
Given('existe un espacio físico llamado SIS502Prueba', async function () {
    this.espacioFisicoExistente4 = new EspacioFisicoDTO("SIS502Prueba", this.aula.id, 25);
...
Then('al consultar la base de datos se observan {string} espacios físicos.', async function (numeroRegistros: number) {
    // Se consulta a la base de datos y se obtiene lo almacenado
    this.registrosAlmacenados = await this.espaciosFisicosController.obtenerEspaciosFisicos();
