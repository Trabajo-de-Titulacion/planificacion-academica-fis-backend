...
Given(
  'que se tiene una asignatura con el código {string}, nombre {string} y cantidad de créditos {string}',
  { timeout: 5 * 5000 },
  async function (
    codigoAsignaturaExistente,
    nombreAsignaturaExistente,
    creditosAsignaturaExistente,
  ) {
    /*
...
When(
  'se ingrese el asignatura con código {string}, nombre {string} y cantidad de créditos {string}',
  { timeout: 5 * 5000 },
  async function (
    codigoAsignaturaNueva,
    nombreAsignaturaNueva,
    creditosAsignaturaNueva,
  ) {
    /*
    Los datos recibidos son:
    |            Significado           | Nombre variable                     | Tipo de dato necesario |
    ...
    this.resultadoIngresarAsignatura =
      await this.asignaturaController.crearUnaAsignatura(this.asignaturaACrear);
  },
);

Then(
  'se obtendrá una respuesta del registro individual {string}.',
  { timeout: 5 * 5000 },
  async function (respuestaIngresoAsignaturaIndividual) {
    /*
    Los datos recibidos son: 
    |          Significado              | Nombre variable                            | Tipo de dato necesario |
    ...
    assert.equal(
      this.resultadoIngresarAsignatura.mensaje,
      respuestaIngresoAsignaturaIndividual,
    );
  },
);
...
When(
  'se ingrese varias asignaturas por medio de un archivo csv {string}',
  { timeout: 5 * 5000 },
  async function (nombreArchivo) {
    ...
    this.resultadoIngresarVariasAsignatura =
      await this.asignaturaController.crearVariasAsignaturas({
...
Then(
  'se obtendrá una respuesta del registro múltiple {string}.',
  { timeout: 5 * 5000 },
  function (respuestaDeMultiplesIngresosAsignaturas) {
    /*
    Los datos recibidos son: 
    |          Significado               | Nombre variable                            | Tipo de dato necesario |
    ...
    // Realizar comprobación
    assert.equal(
      this.resultadoIngresarVariasAsignatura.mensaje,
      respuestaDeMultiplesIngresosAsignaturas,
    );
  },
);
...
After('@RegistroDeVariasAsignaturas', async function () {
  ...
  for (
    let i = 0;
    i < this.resultadoIngresarVariasAsignatura.asignaturasIngresadas.length;
    i++
  ) {
    await getRepository(AsignaturaEntity).delete(
      this.resultadoIngresarVariasAsignatura.asignaturasIngresadas[i],
    );
  }
});
