...
Given(
  'que se tiene un docente llamado {string} con el correo electrónico {string}',
  { timeout: 5 * 5000 },
  async function (nombreDocente, correoElectronico) {
    /*
...
When(
  'se ingrese el docente llamado {string} con el correo electrónico {string}',
  { timeout: 5 * 5000 },
  async function (nombreDocenteNuevo, correoElectronicoNuevo) {
    /*
    Los datos recibidos son:
    |          Significado         | Nombre variable        | Tipo de dato necesario |
    ...
    this.resultadoIngresarDocente = await this.docenteController.crearDocente(
      this.docenteNuevo,
    );
  },
);
...
Then(
  'al leer la base de datos se podrá observar que la clave del docente se generó existosamente',
  { timeout: 5 * 5000 },
  async function () {
    ...
    this.usuarioDeTipoDocenteCreado =
      await this.usuarioService.obtenerUsuarioPorSuCorreo(
        this.docenteNuevo.correoElectronico,
      );
    if (this.usuarioDeTipoDocenteCreado) {
      assert.equal(
        typeof this.usuarioDeTipoDocenteCreado.clave,
        typeof 'string',
      );
    }
  },
...
Then(
  'se obtendrá una respuesta del envío de correo electrónico {string}.',
  async function (respuestaCorreo) {
    /*
    Los datos recibidos son: 
    |      Significado             | Nombre variable  | Tipo de dato necesario |
...
After('@RegistroDeDocente', async function () {
  ...
  if (this.usuarioDeTipoDocenteCreado) {
    ...
    const rolDocente = await getRepository(RolUsuarioEntity).findOne({
      where: { usuario: this.usuarioDeTipoDocenteCreado },
    });
...
When(
  'se ingrese varios docentes por medio de un archivo csv {string}',
  { timeout: 5 * 5000 },
  async function (nombreArchivo) {
    /*
    Los datos recibidos son:
    |       Significado       | Nombre variable  | Tipo de dato necesario |
    ...
    this.resultadoIngresarVariosDocentes =
      await this.docenteController.crearVariosDocentes({
        buffer: this.lecturaArchivo,
      });
  },
);

Then(
  'al leer la base de datos se podrá observar que todos los docentes registrados tienen una clave',
  { timeout: 5 * 5000 },
  async function () {
    ...
    for (
      let i = 0;
      i < this.resultadoIngresarVariosDocentes.docentesIngresados.length;
      i++
    ) {
      this.usuarioDeTipoDocenteCreado =
        await this.usuarioService.obtenerUsuarioPorSuCorreo(
          this.resultadoIngresarVariosDocentes.docentesIngresados[i]
            .correoElectronico,
        );
      if (this.usuarioDeTipoDocenteCreado) {
        assert.equal(
...
Then(
  'se obtendrá una respuesta del envío de correos electrónicos {string}.',
  { timeout: 5 * 5000 },
  async function (respuestaDeMultiplesIngresos) {
    /*
   Los datos recibidos son: 
   |          Significado          | Nombre variable              | Tipo de dato necesario |
    ...
    */
    ...
    //Realizar comprobación
    assert.equal(
      this.resultadoIngresarVariosDocentes.mensaje,
      respuestaDeMultiplesIngresos,
    );
  },
);
...
After('@RegistroDeVariosDocentes', async function () {
  ...
  for (
    ...
    const rol = await getRepository(RolUsuarioEntity).findOne({
