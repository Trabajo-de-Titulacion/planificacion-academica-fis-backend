...
When(
  'se indica el número de estudiantes {int}, {int}, {int} y {int}, respectivamente,',
  async function (
    numEstudiantesAsignatura1: number,
    numEstudiantesAsignatura2: number,
    numEstudiantesAsignatura3: number,
    numEstudiantesAsignatura4: number,
  ) {
    this.numeroEstudiantes1 = new NumeroEstudiantesPorSemestreDTO(
      this.semestre.id,
      this.asignatura1.id,
      numEstudiantesAsignatura1,
    );
    this.numeroEstudiantes2 = new NumeroEstudiantesPorSemestreDTO(
      this.semestre.id,
      this.asignatura1.id,
      numEstudiantesAsignatura2,
    );
    this.numeroEstudiantes3 = new NumeroEstudiantesPorSemestreDTO(
      this.semestre.id,
      this.asignatura1.id,
      numEstudiantesAsignatura3,
    );
    this.numeroEstudiantes4 = new NumeroEstudiantesPorSemestreDTO(
      this.semestre.id,
      this.asignatura1.id,
      numEstudiantesAsignatura4,
    );

    this.numeroEstudiantesPorSemestreController = await this.app.get(
      NumeroEstudiantesPorSemestreController,
    );

    await this.numeroEstudiantesPorSemestreController.registrarNumeroEstudiantesPorSemestre(
      [
        this.numeroEstudiantes1,
        this.numeroEstudiantes2,
        this.numeroEstudiantes3,
        this.numeroEstudiantes4,
      ],
    );
  },
);
...
Then(
  'al consultar la base de datos de número de estudiantes se observan {int} registros.',
  async function (numeroRegistros: number) {
    this.registrosAlmacenados =
      await this.numeroEstudiantesPorSemestreController.obtenerNumeroEstudiantesPorSemestreId(
        this.semestre.id,
      );
